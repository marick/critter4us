...
class TimesliceTests < FreshDatabaseTestCase
  ...
  context "animals" do 
    ...
    context "that should be considered in use at the timeslice moment" do 
      ...
      should "except that those from a given reservation will not be include" do 
  ...
  context "exclusions_due_to_procedure_vs_animal_mismatch" do 
    setup do 
      # Date is relevant because some animals "do not exist" at a given 
      # time (they have been taken out of service), and those are not 
      # included in the return value.
      @date=Date.new(2009, 12, 12)
      @time = MORNING
    end

    should "apply rules to create exclusion map" do
      procedure = Procedure.create(:name => 'venipuncture')
      horse = Animal.create(:name => 'horse')
      cow = Animal.create(:name => 'cow')
      
      map_maker = flexmock("making map between procedures and animals")

      @timeslice.move_to(@date, @time)
      during { 
        @timeslice.exclusions_due_to_procedure_vs_animal_mismatch(map_maker)
      }.behold! {
        @procedure_source.should_receive(:all).once.
                          and_return([procedure])
        @animal_source.should_receive(:all_in_service_on).once.
                       with(@date).
                       and_return([horse, cow])
        map_maker.should_receive(:calculate).once
        map_maker.should_receive(:as_map).once.
                  with([procedure], [horse, cow]).
                  and_return([horse])
      }
      assert_equal([horse], @result)
    end
  end
end
